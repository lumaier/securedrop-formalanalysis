...
if argv[1] in ['Auto_Secrecy_Source_Message']:
  ...
  match = matchAgainstList([
    ...
    '!KU( H(kdf(<\'g\'^(~je_dh_sk*kdf(<\'DH\', ~passphrase>)),             \'g\'^(~je_dh_sk*~x), \'g\'^~x, je_dh_pk, \'g\'^kdf(<\'DH\', ~passphrase>)           >),       \'g\'^~x, z, c_pq))',
    '!KU( kdf(<\'g\'^(~je_dh_sk*kdf(<\'DH\', ~passphrase>)),           \'g\'^(~je_dh_sk*~x), \'g\'^~x, je_dh_pk, \'g\'^kdf(<\'DH\', ~passphrase>)         >))',
    '!KU( ~msg )',
    '!KU( kdf(<\'g\'^(~je_dh_sk*kdf(<\'DH\', ~passphrase>)),',
